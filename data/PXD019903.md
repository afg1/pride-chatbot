...
### Title
Effect of UBASH3B and WHIP genetic deletion on the TNF-RSC

### Description
Determination of TNF-RSC composition in UBASH3B and WHIP KOs.
...
### Data Protocol
Data acquisition. LC-MS/MS analysis was carried out on a QExactive HF Hybrid Quadrupole-Orbitrap mass spectrometer (Thermo Fisher) coupled to an Acquity UPLC M-Class System (Waters). Peptides were separated on a Acquity UPLC M-Class column (15 cm length; 75 uM diameter) with a 120 minutes gradient from ACN 5% to 37% at a flow rate of 300 nL/min. The MS was operated in a data-dependent acquisition mode with the following parameters: one full FTMS scan in the m/z range 350-1500 with resolution of 120000 and maximum injection of 50 ms; precursor ions with charge 2-7 were isolated with a window of 1.2 m/z and fragmented using HCD with a normalized collision energy of 28%; and analyzed in FTMS at 30000 with a maximum injection time of 50 ms. The AGC target was set to 1e5, and dynamic exclusion of 30 seconds. Data analysis. MS1 intensities from shotgun data were analyzed using MaxQuant (v 1.5.2.8; Cox and Mann 2008). UniProtKB/Swiss-Prot database (download 2020.04.01) using the following parameters: fully tryptic peptides; fixed modifications: Carbamidomethyl; variables modifications: acetyl, oxidation (M); MS and MS/MS mass tolerance set to 20ppm; FDR <1% at the protein level; LFQ with a minimum of 2 unique peptides; match-between-runs. Customized R scripts were used to carry out the statistical analysis; steps included median normalization, imputation of missing values using sampling from the 5 lowest quantiles, calculation of fold changes and associated non-adjusted and adjusted (BH) p-values.
