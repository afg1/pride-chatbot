...
### Title
Proteome-wide structural changes measured with 2 limited proteolysis-mass spectrometry: an advanced 3 protocol for high-throughput applications.
...
### Sample Protocol
Experiment 1 (yeast + human alpha-synuclein, protein mixture): the experiment was performed with the BY4742 wild-type strain in shake-flask cultures. Cultures were obtained in YPD medium, pelletting cells at OD 0.8. Monomeric alpha-synuclein (M) and amyloid-like fibrils (F) of alpha-synuclein were obtained as described before (Feng et al., 2014) and spiked into the yeast lysates at 5 pmol/ug (M1 and F1) lysate and 20 pmol/ug lysate (M2 and F2). Each proteome extract was then split into a control sample, which was subjected to only tryptic digestion (to quantify protein abundance changes), and a LiP sample, which was subjected to double-protease digestion step with a nonspecific protease followed by complete digestion with trypsin (to quantify peptide changes due to protein structural changes). 100 μg of extracted proteome was incubated with 1:100 (w/w) of Proteinase K for 5 min at 25°C. Digestion reactions were stopped by heating LiP samples for 5 min at 98°C in a thermocycler followed by denaturation with 5% sodium deoxycholate. The same procedure was applied to control samples with water added instead of Proteinase K. Both LiP and control samples were then subjected to reduction with 5mM TCEP, alkylation with 40mM Iodoacetamide and complete tryptic digestion with trypsin 1:100 (w/w) for 16 h at 37°C. The peptide mixtures were acidified with formic acid and desalted with Sep-Pak tC18 cartridges or 96 wells elution plates. The final desalted peptides were resuspended in 0.1% formic acid and analyzed by mass spectrometry.
...
### Keywords
Human alpha-synuclein, Yeast, Structural proteomics, Shotgun proteomics, Lip-ms
